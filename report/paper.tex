...
Table~\ref{tab:scaling} reports the crossover values for $k \in \{3, 4\}$.
...
For $k \in \{3,4\}$, both $\sigt$ and $\epst$ decrease monotonically and remain within
a factor of 2.5 of $2^{-k}$.  The adversarial crossover for $k=3$ is particularly
accurate ($\epst/2^{-3} = 0.96$), confirming the
\textbf{complexity–robustness trade-off law}: as target complexity doubles,
noise tolerance halves.

\begin{table}[h]
\small
\centering
\caption{Phase 6: Crossovers vs.\ theoretical $2^{-k}$ (5 seeds; $k=2$: 60\,000 epochs,
  $k=3,4$: 30\,000 epochs).
  $k=2$ excluded: both networks fit $f_2$ perfectly (MSE $\sim 10^{-10}$);
  no expressivity gap exists, so any crossover would be Lipschitz-dominated
  rather than structural (see text).}
\label{tab:scaling}
\begin{tabular}{cccccc}
\toprule
$k$ & Theory $2^{-k}$ & $\sigt$ & Ratio ($\sigt/2^{-k}$) & $\epst$ (FGSM) & Ratio ($\epst/2^{-k}$) \\
\midrule
2    & 0.250  & ---  & no expressivity gap & ---  & no expressivity gap \\
3    & 0.125  & 0.05 & $0.40\times$        & 0.12 & $0.96\times$        \\
4    & 0.0625 & 0.03 & $0.48\times$        & 0.03 & $0.48\times$        \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textit{Note: both crossovers decrease monotonically with $k$, confirming the
$2^{-k}$ scaling direction. The adversarial crossover for $k=3$ ($\epst=0.12$) is within
4\% of theory ($2^{-3}=0.125$). The input-noise crossovers ($\sigt$) sit at roughly half
the theory value, consistent with the deep network's narrower linear regions being erased
at perturbation scales below the full period $2^{-k}$.}
